...
from erpnext_chile_factura.erpnext_chile_sii_integration.doctype.ejecutor_autoingreso_pinv.ejecutor_autoingreso_pinv import ejecutar_autoingreso
...
def run_autoingreso_desde_preinvoice(preinvoice_name):
    ...
    ejecutar_autoingreso(
        docname=ejecutor.name,
        preinvoice_names=json.dumps([preinvoice_name])
    )

    return {
        "message": f"Autoingreso ejecutado para {preinvoice_name}",
